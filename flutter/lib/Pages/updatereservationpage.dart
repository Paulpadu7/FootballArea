...
class UpdateReservation extends StatefulWidget{
  ...
  UpdateReservation(this.reservationRepositorty,this.reservation);

  @override
  State<StatefulWidget> createState() => UpdateReservationState(reservationRepositorty, reservation);
}

class UpdateReservationState extends State<UpdateReservation> {
  Reservation reservation;
  ReservationRepositorty reservationRepositorty = new ReservationRepositorty();

  UpdateReservationState(this.reservationRepositorty, this.reservation);
                ...
                // crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [
                    Positioned(
                        ...
                        Column(
                            children: [
                              Text(reservation.match.home_team + " vs " + reservation.match.away_team,style:TextStyle(fontSize: 25)),
                              SizedBox(height: 25),
                              Text(reservation.match.date,style:TextStyle(fontSize: 20)),
                              SizedBox(height: 25),
                      ...
                      MaterialButton(
                          ...
                          onPressed: () {
                            ...
                            if(calculate_total_price()!=0){
                              reservationRepositorty.update(reservation.idReservation, number_of_tickets, calculate_total_price());
  ...
  int calculate_total_price()
  {
    ...
    else
    {
      if(ticketType == TicketType.short_side)
        sum = number_of_tickets * reservation.match.price;
      else{
        if(ticketType == TicketType.long_side)
          sum = number_of_tickets * reservation.match.price * 2;
        else
          sum = number_of_tickets * reservation.match.price * 5;
      }
    }
    return sum;
  }
